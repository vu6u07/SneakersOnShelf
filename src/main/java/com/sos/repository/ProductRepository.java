...
public interface ProductRepository extends JpaRepository<Product, Integer> {
	...
	// Collection Params
	@Query(value = "SELECT new com.sos.dto.CollectionProductDTO(p.id, p.name, i.image, p.sellPrice, p.originalPrice, p.productGender, p.productStatus, p.createDate, p.updateDate) FROM Product p LEFT JOIN p.productImage i JOIN p.productDetails pd WHERE (:query IS NULL OR p.name LIKE :query) AND (COALESCE(:sizeName) IS NULL OR (pd.size IN (:sizeName) AND pd.activeStatus = 'ACTIVE' AND pd.quantity > 0)) AND (COALESCE(:brandId) IS NULL OR p.brand.id IN (:brandId)) AND (COALESCE(:categoryId) IS NULL OR p.category.id IN (:categoryId)) AND (COALESCE(:colorId) IS NULL OR p.color.id IN (:colorId)) AND (COALESCE(:soleId) IS NULL OR p.sole.id IN (:soleId)) AND (COALESCE(:materialId) IS NULL OR p.material.id IN (:materialId)) AND (COALESCE(:shoeHeight) IS NULL OR p.shoeHeight IN (:shoeHeight)) AND (COALESCE(:benefit) IS NULL OR p.benefit IN (:benefit)) AND (COALESCE(:shoeFeel) IS NULL OR p.shoeFeel IN (:shoeFeel)) AND (COALESCE(:surface) IS NULL OR p.surface IN (:surface)) AND (COALESCE(:productGender) IS NULL OR p.productGender IN (:productGender)) AND (:productStatus IS NULL OR p.productStatus = :productStatus) GROUP BY p")
	Page<CollectionProductDTO> findCollectionProductDTO(String query, Collection<String> sizeName, Collection<Integer> brandId,
	...
	@Query(value = "SELECT new com.sos.dto.CollectionProductDTO(p.id, p.name, i.image, p.sellPrice, p.originalPrice, p.productGender, p.productStatus, p.createDate, p.updateDate) FROM OrderItem od JOIN od.order o JOIN od.productDetail pd JOIN pd.product p LEFT JOIN p.productImage i WHERE o.orderStatus = 'APPROVED' AND od.orderItemStatus = 'APPROVED' AND (:query IS NULL OR p.name LIKE :query) AND (COALESCE(:sizeName) IS NULL OR (pd.size IN (:sizeName) AND pd.activeStatus = 'ACTIVE' AND pd.quantity > 0)) AND (COALESCE(:brandId) IS NULL OR p.brand.id IN (:brandId)) AND (COALESCE(:categoryId) IS NULL OR p.category.id IN (:categoryId)) AND (COALESCE(:colorId) IS NULL OR p.color.id IN (:colorId)) AND (COALESCE(:soleId) IS NULL OR p.sole.id IN (:soleId)) AND (COALESCE(:materialId) IS NULL OR p.material.id IN (:materialId)) AND (COALESCE(:shoeHeight) IS NULL OR p.shoeHeight IN (:shoeHeight)) AND (COALESCE(:benefit) IS NULL OR p.benefit IN (:benefit)) AND (COALESCE(:shoeFeel) IS NULL OR p.shoeFeel IN (:shoeFeel)) AND (COALESCE(:surface) IS NULL OR p.surface IN (:surface)) AND (COALESCE(:productGender) IS NULL OR p.productGender IN (:productGender)) AND (:productStatus IS NULL OR p.productStatus = :productStatus) GROUP BY p ORDER BY SUM(od.quantity) DESC")
	Page<CollectionProductDTO> findBestSellingProductDTO(String query, Collection<String> sizeName, Collection<Integer> brandId,
			Collection<Integer> categoryId, Collection<Integer> colorId, Collection<Integer> soleId,
			Collection<Integer> materialId, Collection<ShoeHeight> shoeHeight, Collection<Benefit> benefit,
			Collection<ShoeFeel> shoeFeel, Collection<Surface> surface, Collection<ProductGender> productGender,
			ProductStatus productStatus, Pageable pageable);
