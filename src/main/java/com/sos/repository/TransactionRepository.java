...
public interface TransactionRepository extends JpaRepository<Transaction, Integer> {
	...
	@Query(value = "SELECT new com.sos.dto.TransactionDTO(t.id, COALESCE(a.fullname, ''), o.id, t.transactionType, t.transactionStatus, t.paymentMethod, t.amount, t.createDate, t.updateDate) FROM Transaction t LEFT JOIN t.staff a JOIN t.order o WHERE (:query IS NULL OR (o.id LIKE :query OR a.fullname LIKE :query)) AND (:transactionType IS NULL OR t.transactionType = :transactionType) AND (:paymentMethod IS NULL OR t.paymentMethod = :paymentMethod)")
